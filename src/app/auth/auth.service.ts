...
export class AuthService {
  ...
  public verifyCode(body: object) {
    return this._requestApiService.postApi(`${UrlApi.AUTH_VERIFY_CODE}`, body).pipe(
      map((res) => {
        return res;
      })
    );
  ...
  public register(body: object) {
    return this._requestApiService.postApi(`${UrlApi.AUTH_REGISTER}`, body).pipe(
      map((res) => {
        return res;
      })
    );
  ...
  public forgotPassword(body: object) {
    return this._requestApiService.postApi(`${UrlApi.AUTH_FORGOT_PASSWORD}`, body).pipe(
      map((res) => {
        return res;
      })
    );
